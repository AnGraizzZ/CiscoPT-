# Домашнее задание к занятию 1 «Disaster recovery и Keepalived» Чепелев Павел
...
------
[Схема](pkt/hsrp_advanced1.pkt) 


Я перенастроил роутер 2 чтобы он отслеживал подключение на нулевом порту, если оно обрывается, то приоритет на первом порту падает на 10 (router-2.png)
![router-2.png](img/router-2.png)

но мне пришлось перенастроить приоритет роутера 1 в группе 1, изменив с 50 на 95 (router-1.png). Потому что приоритет на 2 роутере падал только на 10 единиц, до 90 соответственно
![router-1.png](img/router-1.png)
### Задание 2
